    ...
    class Server
      ...
      class AccessGrant
        ...
        def authorize!(expires_in = nil)
          ...
          self.class.collection.update_one({:_id=>code, :access_token=>nil, :revoked=>nil}, {:$set=>{ :granted_at=>granted_at, :access_token=>access_token.token }} )
          reload = self.class.collection.find({:_id=>code, :revoked=>nil}, { 'projection' => { 'access_token' => 1 } }).limit(1).first
